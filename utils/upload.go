...
func (*Upload) UploadFile(file *multipart.FileHeader) (string, string, error) {
	// 读取文件后缀
	...
	// 尝试创建此路径
	mkdirErr := os.MkdirAll(global.CONFIG.Upload.StorePath, os.ModePerm)
	if mkdirErr != nil {
	...
	// 拼接路径和文件名
	p := global.CONFIG.Upload.StorePath + "/" + filename
	filepath := global.CONFIG.Upload.Path + "/" + filename
...
func (*Upload) DeleteFile(key string) error {
	// 检查 key 是否为空
	if key == "" {
	...
	p := filepath.Join(global.CONFIG.Upload.StorePath, key)
